...
-----------------------------  Test Hoare Semantics  ---------------------------------


 -- Following functions are used for testing Hoare sematics
 -- ProducerConsumer ()
 --       Creates 3 consumer and 5 producer threads and initializes boundedBuffer
 --  Producer()
 --       Calls Producer() method of BoundedBuffer class
 --  Consumer()
 --       calls Consumer() method of BoundedBuffer class
 -- Please note that below code of bounded buffer is adapted from lecture slides, however it is modified to suit the 
 -- requirements of testing

  var
    -- Array of threads for producer and consumer
    thArray: array [8] of Thread = new array of Thread { 8 of new Thread }
    -- Initialize BoundedBufferMonitor
    boundedBuffer: BoundedBufferMonitor = new BoundedBufferMonitor
  ...
  function ProducerConsumer ()
   ...
   function Producer (p: int)
     var i: int

     -- Call Monitor's Producer method
     for i = 0 to 4
       boundedBuffer.Producer (p)
     endFor
   endFunction

   function Consumer (c: int)
     var i: int

     -- Call Monitor's Consumer method
     for i = 0 to 4
       boundedBuffer.Consumer (c)
     endFor
   endFunction

-----------------------------  BoundedBufferMonitor  ---------------------------------
  
  behavior BoundedBufferMonitor
  -- Monitor implementation of BoundedBuffer using Hoare Semantics
  -- In this implementation, as and when producer and consumer complete their 
  -- task of producing or consuming, they signal each other
  -- Implements below method:
  -- Init()
  --   Initializes Hoare condition variables and monitor mutex, array of buffer
  -- Producer()
  --   Checks if buffer is full, if yes wait on monitor's condition variable
  --   If no, produce a character, add it to buffer and signal consumer to consume
  -- Consumer()
  --   Checks if buffer is empty, if yes, wait on monitor's condition variable
  --   If no, consume character from buffer and signal producer to produce more
  --   characters.


   method Init () 
     notEmpty = new Condition1
     notEmpty.Init ()
     notFull = new Condition1
     notFull.Init ()
     monitorMutex = new HoareMutex
     monitorMutex.Init ()
  ...
  method Producer(p: int)
    var
     c: char = intToChar ('A' + p - 1)

    -- Acquire exclusive acccess to the monitor's entry method
    monitorMutex.Lock ()
    ...
    -- Check if buffer is full, if yes, wait on notFull condition variable
    -- till signalled
    if cntFull == BUFFER_SIZE
     notFull.Wait (&monitorMutex)
    endIf
    ...
    -- if there is any consumer waiting to consume the character,
    -- signal the consumer
    notEmpty.Signal (&monitorMutex)

    -- Release the monitor's mutex before exit
    -- Ideally, signal method would handover the mutex to thread waiting on 
    -- Monitor's condition. However, if this is the very 1st thread to run, then
    -- signal call will not be able to handover mutex to any waiting thread
    -- In that case, currentThread must call unlock to release mutex.
  ...
  method Consumer(c: int)
    ...
    -- character and signal 
    if cntFull == 0
     notEmpty.Wait(&monitorMutex)
    endIf
   ...
   -- If there is any producer waiting to produce character
   -- signal it
    notFull.Signal (&monitorMutex)

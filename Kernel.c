...
-----------------------------  Hoare Mutex  ---------------------------------
 behavior HoareMutex
    -- This class is implemented as a part of Hoare Semantics implementation
    -- Rest of the method implementation is same as the previously implemented
    -- Mutex. This class provides an extra method named Give ().
    -- Details of each method is as follows:
    --    Give(t: ptr to Thread)
    --         Give away the mutex lock to the siganlled thread so that the if any
    --         other thread tries to acquire the lock on monitor's entry method,
    --         it would wait on the waiting list of mutex. Memory invariance is
    --         is preserved across signal to wakeup from wait
    --    Lock()
    --         Acquire the mutex if free, otherwise wait until the mutex is
    --         free and then get it.
    --    Unlock()
...
----------------------------- Hoare Condition  ---------------------------------
  ...
  behavior Condition1
   -- This class is used to implement Hoare semantics of condition
   -- variables.
   -- It implements following methods:
   -- Init()
   --     This method initializes the waiting list of threads.
   --     This list will contain all the threads that are waiting on
   --     monitor's condition variable
   --     
   -- Wait(mutex: ptr to HoareMutex)
   --     This method releases the mutex lock of monitor assuming
   --     that it is already locked and goes to sleep waiting to receive 
   --     signal on the condition variable. After receiving the signal,
   --     it assumes that it has already acquired the lock and starts 
   --     executing.
   -- Signal(mutex: ptr to HoareMutex)
   --      This method handsover the monitor's mutex lock to the signalled 
   --      thread. It first removes the thread from the waiting list of 
   --      condition variable, updates its status to READY and adds it to 
   --      readyList.
      ...
      method Wait (mutex: ptr to HoareMutex)
          ...
          -- Calling thread will release the lock before going to sleep
          mutex.Unlock ()

          -- Add the calling thread to condition variable's waiting list before sleeping
          waitingThreads.AddToEnd (currentThread)
          currentThread.Sleep ()
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
   ...
   -- This class implements Thread Manager
   -- In this class, we have few resource threads.
   -- Testing threads can request a resource threads 
   -- by calling GetANewThread() method.
   -- Scheduler will invoke FreeThread() when a thread terminates. 
   -- Thread manager will monitor allocation and
   -- deallocation of resource threads carefully.
   -- It implements following methods:
   -- Init()
   --    This method initializes an array of threads. Each
   --    thread in the array will have status as UNUSED. Initializes
   --    monitor's mutex named threadManagerLock and monitor's
   --    condition variable named aThreadBecameFree
   -- GetANewThread()
   --    It is an entry level method of monitor. When a testing thread
   --    needs a resource threads, it can acquire one by calling this method
   --    If any thread is available on the list of freee threads, method will return
   --    a pointer to free thread, else the requesting thread will wait on monitor's condition
   --    variable. Resource thread's status is set as JUST_CREATED before returning its 
   --    pointer to requesting thread.
   -- FreeThread(t: ptr to Thread)
   --    Entry level method of monitor
   --    Moves thread whose pointer is passed as an argument back to list of free threads
   --    Before adding to the list, sets thread's status to UNUSED
   --    Signals a thread waiting on monitor's condition variable aThreadBecameFree if any
   -- Monitor's both entry level methods acquire monitor's mutex lock before begining execution
   -- and release it before exiting
   -- Print()
   ...
   --
      ...
      method Init ()
        ...
        var  
         ...
         -- Initializing the Array of threads
        ...
        -- Initialize each thread and add it to freeList, assign initial status as  UNUSED
	  ...
	  threadManagerLock.Lock ()				--Acquire excusive access to entry method using monitor's mutex lock
          
	  while freeList.IsEmpty ()  				--While no free thread is available, wait on condition variable     
            aThreadBecameFree.Wait (&threadManagerLock)
         endWhile

          th = freeList.Remove ()				--When thread becomes available in the freeList, allocate it to the requesting process
          th.status = JUST_CREATED				--Set the status of the thread as JUST_CREATED before returning it to the requesting process
          --self.Print ()				        --Print status of the threadTable
          
          threadManagerLock.Unlock ()				--Release the monitor lock
          return th
        endMethod
	...
	threadManagerLock.Lock ()				--Acquire excusive access to entry method using monitor's mutex lock
        ...
        th.status = UNUSED					--Set the status of the thread to UNUSED
        freeList.AddToEnd (th)					--Add the resource thread back to the list of free threads

   	aThreadBecameFree.Signal (&threadManagerLock)		--Signal the 1st thread on the waiting list of the condition variable

	--self.Print ()						--Print status of the threadTable
        threadManagerLock.Unlock ()				--Unlock the monitor's mutex
        endMethod
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
    -- This class is monitor implmentation of Process manager which manages allocation and deallocation
    -- of ProcessControlBlocks to requesting threads
    -- The class implments following methods:
    -- Init()
    --     Initializes ProcessControlBlock array. Initially, the status of each ProcessControlBlock
    --     is FREE, hence they are added to the freeList
    -- GetANewProcess()
    --     Entry level method of monitor. Allocates ProcessControlBlock to the requesting thread if available, else 
    --     the requesting thread waits on monitor's condition variable  aProcessBecameFree. Once PCB is available,
    --     assign a pid, set its status to ACTIVE, and return its pointer.
    -- FreeProces(p: ptr to ProcessControlBlock)
    --     Entry level method of monitor.Adds ProcessCOntrolBlock back to the array of freeList.
    --     Before adding, sets its status to FREE, and assigns -1 as PID. Siganls monitor's condition
    --     variable aProcessBecameFree to indicate the availability of free PCB
    -- Print()
    --     Print all processes.  Since we look at the freeList, this
    --     routine disables interrupts so the printout will be a
    -- 	   consistent snapshot of things.
    -- PrintShort()
    --    Similar to Print, it prints all processes.
      ...
      method Init ()
        ...
        -- Initialize the ProcessControlBlock array
         processTable = new array of ProcessControlBlock { MAX_NUMBER_OF_PROCESSES of new ProcessControlBlock}
         
        -- Initialize each ProcessControlBlock in the array
	for i = 0 to 9
          processTable [i].Init ()
        endFor

        -- Initialize the ProcessManager's monitor mutex lock
	...
	-- Initialize the Condition variables
  	aProcessBecameFree = new Condition
        ...
        -- Add all ProcessControlBlocks to the freeList
        for i = 0 to MAX_NUMBER_OF_PROCESSES - 1
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        --
        -- This method returns a new ProcessControlBlock; requesting thread
        -- will wait until one is available.
        --
         var
	   p: ptr to ProcessControlBlock
  
	 processManagerLock.Lock ()				-- Acquire the Monitor's mutex lock to have exclusive access to monitor's entry method
        
         while freeList.IsEmpty ()  				-- While there are no ProcessControlBlocks available in the freeList, wait till signalled                             
	  aProcessBecameFree.Wait (&processManagerLock)
         endWhile
         
         p = freeList.Remove()					-- Remove a ProcessControlBlock from the list of free ProcessControlBlocks        

         nextPid = nextPid + 1					
         p.pid = nextPid					-- Assign a Process ID to the Process ID
        
         p.status = ACTIVE					-- Set the process's status to ACTIVE
         
         --self.Print ()
         processManagerLock.Unlock ()				-- Release the monitor's mutex lock
	...
	p.status = FREE						-- Set the process's status to FREE
        p.pid = -1						-- Set the process ID to -1
	freeList.AddToEnd (p)					-- Add the Process to the freeList
        aProcessBecameFree.Signal (&processManagerLock)		-- Signal the waiting thread 
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
     -- The FrameManager class manages allocation and deallocation of
     -- frames to threads. Monitor implementation
     -- This class implements following method:
     -- Init()
     --    Initializes framesInUse bitmap, along with monitor's mutex lock 
     --    frameManagerLock and condition variables leader,newFramesAvailable
     --    framesInUse bitmap is used to keep a track of allocated and free frames
     -- GetNewFrames(aPageTable: ptr to AddrSpace, numFramesNeeded: int)
     --    Monitor's entry level method
     --    Allocates number of needed frames to the requesting thread
     --    Uses 2 condition variables to solve the problem of starvation
     --    Requesting thread 1st checks if there are any threads waiting to acquire frames,
     --    if yes, then wait on newFramesAvailable condition variable. This will
     --    maintain the list of all the threads waiting  to get ahead of the line
     --    Once ahead of the line, check if required number of frames are available,
     --    if yes, acquire them, adjust the number of available frames and signal threads
     --    waiting to get ahead of line on newFramesAvailable variable. If not available, then
     --    wait on leader condition variable. 
     -- ReturnAllFrames (aPageTable: ptr to AddrSpace)
     --    Monitor's entry level method
     --    Marks all the frames that are being returned as free in bitmap. Once done, signals
     --    the leader thread to inform that some frames are free now
     -- GetAFrame () returns int
     ...
     --    Print which frames are allocated and how many are free.
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
       --
       -- This method allocates numFramesNeeded to the requesting thread
       --

	var 
         i, f, frameAddr: int

	-- Acquire exclusive access to this entry level method of monitor
	  frameManagerLock.Lock()	
         
          waitingThreads = waitingThreads + 1
        
        -- Check if there are threads waiting in the line, if yes, join the join
        -- else you become the leader thread 
          if waitingThreads > 1
  	   newFramesAvailable.Wait (&frameManagerLock)
          endIf          


        -- Leader thread runs.Checks if required number of frames are available, 
        -- wait on monitor's leader condition variable if not available
          while (numFramesNeeded > numberFreeFrames)
            leader.Wait (&frameManagerLock)					
          endWhile

         -- Once required number of frames are available, 
	 -- allocate them to the requesting thread
          for i = 0 to numFramesNeeded - 1
	   -- Find a free frame and set its bit
	    f = framesInUse.FindZeroAndSet ()	

	   -- Find the address of the free frame			
            frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (f * PAGE_SIZE)
       
           -- Store the address of the allocated frame
	...
	-- Update the AddSpace's numberOfPages field by assigning the number of frames allocated to it
          aPageTable.numberOfPages = numFramesNeeded

	-- Remove the the requesting thread from the waitingThreads count 
	  waitingThreads = waitingThreads - 1
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
       --
       -- This method clears the bit for all the returned frames
       --

         var 
           i, frameAddr, bitNumber, numFramesReturned: int
	...
	frameManagerLock.Lock()          
	  ...
	  -- Find the Physical Address of the free frame
	     frameAddr = aPageTable.ExtractFrameAddr (i)

	  -- Find the bitNumber of frame being returned
             bitNumber = (frameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME) / PAGE_SIZE

	  -- Clear bit for the frame in framesInUse bitmap
             framesInUse.ClearBit (bitNumber)
          endFor
	
        -- Update the number of free frames
        numberFreeFrames = numberFreeFrames + numFramesReturned

        -- Signal thread that is waiting ahead of the line
        leader.Signal (&frameManagerLock)
